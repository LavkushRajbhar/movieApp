...
" alt="parasite"
                class="hover:opacity-75 w-64 lg:w-96 transition ease-in-out duration-150">
...
<div class="mt-4">
    <a href="{{route('movies.show',$movie['id'])}}">
        <img src="{{$movie['poster_path']}}"
                class="hover:opacity-75 transition ease-in-out duration-150" alt="poster">
    </a>
    <a href="{{route('movies.show',$movie['id'])}}" class="text-sm leading-normal block text-gray-400 hover:text-white mt-1">{{$movie['title']}}
    </a>
</div>
@endforeach
                </div>
...
<div class="credits border-b border-gray-700 bg-gradient-to-r from-gray-900 via-gray-800 to-gray-900 text-white">
    <div class="container mx-auto px-6 py-12">
        <h2 class="uppercase tracking-wider text-4xl font-extrabold text-gray-100 mb-8 ">Credits</h2>
        @foreach ($credits->groupBy('release_year') as $year => $movies)
        <div class="mb-10">
            <h3 class="text-2xl font-semibold text-gray-400 mb-4 border-b border-gray-700 pb-2 px-4">{{$year}}</h3>
            <ul class="divide-y divide-gray-700">
                @foreach ($movies as $credit)
                <li class="flex items-center justify-between py-4 px-16 transition-transform duration-300 hover:scale-105">
                    <div class="flex items-center">
                        <span class="font-semibold text-lg text-white">{{$credit['title']}}</span>
                    </div>
                    <div class="text-gray-300 italic text-sm">as {{$credit['character']}}</div>
                </li>
                @endforeach
            </ul>
        </div>
